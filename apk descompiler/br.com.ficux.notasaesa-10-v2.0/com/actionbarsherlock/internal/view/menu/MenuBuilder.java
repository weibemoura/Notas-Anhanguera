...
public class MenuBuilder implements Menu {
    ...
    private static int getOrdering(int categoryOrder) {
        ...
        if (index >= 0 && index < sCategoryToOrder.length) {
            return (sCategoryToOrder[index] << 16) | (Menu.USER_MASK & categoryOrder);
        }
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!(shortcutsVisible && this.mResources.getConfiguration().keyboard != 1 && this.mResources.getBoolean(C0029R.bool.abs__config_showMenuShortcutsWhenKeyboardPresent))) {
            z = false;
        }
    ...
    public boolean bindNativeOverflow(android.view.Menu menu, OnMenuItemClickListener listener, HashMap<android.view.MenuItem, MenuItemImpl> map) {
        ...
        for (MenuItemImpl nonActionItem : nonActionItems) {
            if (nonActionItem.isVisible()) {
                ...
                if (nonActionItem.hasSubMenu()) {
                    ...
                    Iterator it = ((SubMenuBuilder) nonActionItem.getSubMenu()).getVisibleItems().iterator();
                    while (it.hasNext()) {
                        MenuItemImpl subItem = (MenuItemImpl) it.next();
